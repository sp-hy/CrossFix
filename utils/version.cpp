...
std::string GetExecutableVersion(const char* exePath) {
    ...
    UINT fileInfoSize = 0;
    if (!VerQueryValueA(versionInfo.data(), "\\", (LPVOID*)&fileInfo, &fileInfoSize)) {
        return "";
    }
    ...
    char versionStr[32];
    sprintf_s(versionStr, "%d.%d.%d.%d", majorVersion, minorVersion, buildVersion, revisionVersion);
...
bool CheckExecutableVersion(const char* exePath, const char* expectedVersion) {
    ...
    if (actualVersion.empty()) {
        std::cout << "Warning: Could not read version information from executable" << std::endl;
        return false;
    }
