...
std::set<uint64_t>
    g_recentDumps; // Track recently dumped textures to avoid rapid re-dumps
...
uint64_t HashTexture(const D3D11_TEXTURE2D_DESC *pDesc,
  ...
  if (pInitialData && pInitialData->pSysMem && pDesc) {
    ...
    // Calculate actual buffer size: rowPitch * (height - 1) + actual last row
    // size The last row doesn't need padding, so we can't assume full rowPitch
    // for all rows
    size_t dataSize;
    if (isBlockCompressed) {
    ...
    // For large textures, only sample a portion to reduce crash risk and
    // improve performance This is a trade-off: less unique hashes but much
    // safer on problematic memory
...
void DumpTexture2D(ID3D11Device *pDevice, ID3D11DeviceContext *pContext,
  ...
  // Build temp filepath (we'll move to final location after checking
  // transparency)
