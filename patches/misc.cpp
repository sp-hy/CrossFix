...
bool ApplyVerticalBorderRemoverPatch(uintptr_t base) {
	...
	if (!WriteMemory(addr, &patchValue, sizeof(float))) {
		std::cout << "Failed to patch vertical border remover address (0x" << std::hex << addr << ")" << std::dec << std::endl;
		return false;
	}
	
	std::cout << "Vertical border remover patch applied" << std::endl;
#ifdef _DEBUG
	std::cout << "  Patched 0x" << std::hex << addr << std::dec << " with float value 0.0" << std::endl;
#endif
	return true;
}
