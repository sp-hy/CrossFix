...
// We track the ORIGINAL unscaled values and only scale if the value has changed from last time
extern "C" __declspec(naked) void __stdcall ScaleDialogValues(float* dialogWidth, float* cursorX) {
	__asm {
		push ebp
		mov ebp, esp
...
bool ApplyDialogPatch(uintptr_t base) {
	...
	// Original: movss xmm0,[eax+CHRONOCROSS.exe+1089E14] (F3 0F 10 80 14 9E 08 01)
	// We hook to scale cursor X position and dialog width based on aspect ratio
	// ============================================================
	uintptr_t cursorPosHookAddr = base + 0x433BA;
	uintptr_t cursorPosReturnAddr = cursorPosHookAddr + 8; // After the 8-byte instruction

	// Allocate executable memory for cursor position hook
	g_cursorPosHookMem = VirtualAlloc(NULL, 256, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	...
	// lea edx,[eax+base+0x1089E14] - calculate address of cursorX (8D 90 [offset])
	cursorHookCode[cursorOffset++] = 0x8D;
	cursorHookCode[cursorOffset++] = 0x90;
	memcpy(&cursorHookCode[cursorOffset], &cursorXOffset, 4);
	cursorOffset += 4;
	
	// lea ecx,[eax+base+0x1089E10] - calculate address of dialogWidth (8D 88 [offset])
	cursorHookCode[cursorOffset++] = 0x8D;
	...
	int32_t cursorJmpRel = (int32_t)(cursorPosReturnAddr - (cursorJmpFromAddr + 4));
	...
	int32_t cursorHookRel = (int32_t)((uintptr_t)g_cursorPosHookMem - (cursorPosHookAddr + 5));
